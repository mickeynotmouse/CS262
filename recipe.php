...
function updateRecipe() {
    ...
    if ($image_path) {
        $stmt = $db->prepare('UPDATE recipes SET title=?, instructions=?, servings=?, image_path=? WHERE id=?');
        $stmt->bind_param('ssisi', $title, $instructions, $servings, $image_path, $id);
    } else {
...
function uploadImage() {
    ...
    $file      = $_FILES['image'];
    error_log('File type: ' . $file['type'] . ' Size: ' . $file['size']);
    ...
    error_log('File size: ' . $file['size'] . ' Max: ' . $maxSize);
